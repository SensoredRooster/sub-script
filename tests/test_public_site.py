...
def test_public_site_relative_links_resolve():
    ...
    for page in pages:
        html = page.read_text(encoding="utf-8")
        for href in ("href=\"./\"", "href=\"../\"", "href=\"terms/\"", "href=\"../terms/\"", "href=\"privacy/\"", "href=\"../privacy/\""):
            if href in html:
                target = href.split('href="', 1)[1].split('"', 1)[0]
                assert (page.parent / target / "index.html").is_file() if target.endswith("/") else (page.parent / target).is_file()
...
def test_public_site_contains_no_known_credential_files_or_values():
    ...
    for marker in ("client_secret", "client_secrets.json", "access_token", "refresh_token", "credentials.json", "token.json"):
